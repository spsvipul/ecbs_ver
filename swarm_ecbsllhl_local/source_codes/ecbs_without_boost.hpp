...
void filler(swarm::Timestamp ts, HighLevelNode* node,  size_t i , int hlsnum){
  ...
  if(child_prio>= swarm::superTimestamp()){ 
   //swarm::info("inside filler enqup %i ::::%x", hlsnum,swarm::superTimestamp());
    ...
    swarm::deepen();
    swarm::enqueue(main_loop_task_para, child_prio,
...
void enq_upper(swarm::Timestamp ts, HighLevelNode* node,  size_t i ,  int hlsnum) {
			...
			//if(node->focalHeuristic > node->parentFH || node->cost > node->parentCost)
			//if(node->focalHeuristic > node->parentFH)
				//return;
			// this is now best
      ...
      /*
			if (node->parentTimestamp > child_prio)
		  		child_prio = node->parentTimestamp;
      ...
      */
      ...
      if(ts>= swarm::superTimestamp()){ 
        ...
        swarm::enqueue(main_loop_task_para, ts,EnqFlags::NOHINT,node->id);  
			...
			//node->timestamp = tt+ 1+ child_prio;
      //node->timestamp = 200;
			//node->timestamp = node->parentTimestamp+1;

			//node->timestamp = ROBOT_NUMS + 1 + node->focalHeuristic;

			//node->timestamp = ROBOT_NUMS * node->generation + node->cost;
      //swarm::undeepen();
      //node->timestamp = 500 + node->timestamp;
      //swarm::info("fractal doain ts = %i, nodets: %i, hlsnum: %i ", tt, child_prio , hlsnum );
/*
      swarm::enqueue(main_loop_task_para, node->cost,
					//ROBOT_NUMS * node->generation + node->focalHeuristic, 
					EnqFlags (NOHINT | SUPERDOMAIN),   
					node->id);
*/       
}
...
#else 
*/
void LL_pll(swarm::Timestamp ts,AstarNode current,const LLNodee  prev, LowLevelEnvironment  m_env, came_from (*cameFrom)[32][32][50]) {
...
//#endif 
      ...
      //have_prec[current.state.x][current.state.y][newnodeid]=true;
      
      
      if (m_env.isSolution(current.state)) { 
        //cameFrom[newnodeid].erase(current.state); 
        /*
        ...
        */
        ...
        ///*
        (*cameFrom)[current.state.x][current.state.y][current.state.time].curr_state=current.state;
        (*cameFrom)[current.state.x][current.state.y][current.state.time].neighbor=prev.neighbor;
        (*cameFrom)[current.state.x][current.state.y][current.state.time].gscore=prev.gscore;
        (*cameFrom)[current.state.x][current.state.y][current.state.time].set=true;    
        done[newnodeid]=true; 
        //*/
        ...
        while (i>0) { 
          ...
          solution->add(StateCost((*cameFrom)[curr_state.x][curr_state.y][curr_state.time].curr_state,
          (*cameFrom)[curr_state.x][curr_state.y][curr_state.time].gscore));                                                                                                
					//solution->add(ActionCost(std::get<1>(iter->second), std::get<2>(iter->second)));
          solution->add(ActionCost((*cameFrom)[curr_state.x][curr_state.y][curr_state.time].neighbor.action,
          (*cameFrom)[curr_state.x][curr_state.y][curr_state.time].neighbor.cost));                                                     
        ...
        swarm::enqueue(filler,ts_tem,
		    EnqFlags (NOHINT | PARENTDOMAIN),   
			  hls_node,c_idx,newnodeid); 
    ...
    if (cameFrom[current.state.x][current.state.y][current.state.time][newnodeid].set==false){ 
      ...
      (*cameFrom)[current.state.x][current.state.y][current.state.time].gscore=prev.gscore;
      (*cameFrom)[current.state.x][current.state.y][current.state.time].set=true;
      //cameFrom[current.state.x][current.state.y][current.state.time][newnodeid].path_len= cameFrom[prev.neighbor.state.x][prev.neighbor.state.y][prev.neighbor.state.time][newnodeid].path_len +1;
      ...
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
         // not included in closedSet
          ...
          Cost tentative_gScore = current.gScore + neighbor.cost;
            ...
            /*
            if (neighbor.state.x== current.state.x && neighbor.state.y== current.state.y ){
            //swarm::info("ll_pll enqueue: %i, curr: %i ncurr coordinate... x: %i, y : %i .... parent coordinate x: %i y: %i....tentative g score: %i", fScore, curr,neighbor.state.x,neighbor.state.y,current.state.x,current.state.y,tentative_gScore);
            swarm::enqueue(LL_pll,curr_ts ,EnqFlags::NOHINT, tmp,prevv,m_env);  
            }
            */
            
            
            //cameFrom[neighbor.state.x][neighbor.state.y][neighbor.state.time][newnodeid].path_len= tentative_gScore;
            //swarm::info("ll_pll enqueue child before %i, curr: %i neighbor x: %i,  neighbor y : %i", fScore, curr,neighbor.state.x,neighbor.state.y);
            //if (have_prec[neighbor.state.x][neighbor.state.y][newnodeid]==false){
            //swarm::info("ll_pll enqueue: %i, curr: %i ncurr coordinate... x: %i, y : %i .... parent coordinate x: %i y: %i....tentative g score: %i", fScore, curr,neighbor.state.x,neighbor.state.y,current.state.x,current.state.y,tentative_gScore);
            swarm::enqueue(LL_pll,fScore ,EnqFlags::NOHINT, tmp,prevv,m_env, cameFrom);  
            //}
            //(1000*focalHeuristic + 100*fScore + tentative_gScore)    , cameFrom
  ...
  void lls_search(swarm::Timestamp score, float m_w, const State* startState, 
    ...
    //prev.m_env = LowLevelEnvironment(g_env, c_idx, hls_node->constraints[c_idx], hls_node->solution);
		...
		swarm::enqueue(LL_pll,0,EnqFlags::NOHINT, tmp,prev ,m_env, &cameFrom);
			// most good
			//node->timestamp = ROBOT_NUMS * node->generation + node->focalHeuristic;
	}
