...
//earlier  unordered map was being used but it was giving too many aborts
//so, changed to a fixed size array. array is 4D because current A* have 3D search (X,Y,Time)
//time is the additional state 
...
//solution for time stamp inversion problem
...
void LL_pll(swarm::Timestamp ts,AstarNode current,const LLNodee  prev, LowLevelEnvironment  m_env) {
    ...
    if(done[newnodeid]==true) { //if soln found return
      //swarm::info("return from 1");
      return;
    }
    
    if (m_env.isSolution(current.state)) { //logic to store path when solution found 
      ...
      swarm::enqueue(filler,ts_tem,EnqFlags (NOHINT | PARENTDOMAIN),hls_node,c_idx,newnodeid); //PARENTDOMAIN flag to go from Low level search (LLS) domain to High level search (HLS) domain
    }
    ...
    if (cameFrom[current.state.x][current.state.y][current.state.time][newnodeid].set==false){ 
      ...
      m_env.getNeighbors(current.state, neighbors); //find neighbors of current state
      //the neighbor state means (x,y,t) have (x,y,t+1) and (x+-1,y+-1,t+1) as its neighbor state 
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
        ...
        Cost focalHeuristic = current.focalHeuristic + m_env.focalStateHeuristic(neighbor.state, tentative_gScore) +
  ...
  void lls_search(swarm::Timestamp score, float m_w, const State* startState, 
    ...
    prev.c_idx= c_idx; 
		...
		swarm::deepen(); //create fractal domain for Low level search
    
		swarm::enqueue(LL_pll,0,EnqFlags::NOHINT, tmp,prev ,m_env);
	}
 
void star_t(swarm::Timestamp tt, int si){//dummy init task
...
#endif //swarm case
    swarm::enqueue(star_t,0 , EnqFlags::NOHINT, si); //start enqueue from dummy function
#else//no swarm case
