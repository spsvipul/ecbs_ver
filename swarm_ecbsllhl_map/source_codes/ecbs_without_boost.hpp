...
void enq_upper(swarm::Timestamp ts, HighLevelNode* node,  size_t i ,  int hlsnum) {
      ...
      //swarm::info("fractal domain ts = %i,perentts: %i,  nodets: %i, hlsnum: %i , superts: %x", ts,node->parentTimestamp, child_prio , hlsnum,swarm::superTimestamp() );
    ...
    if (cameFrom[newnodeid].find(current.state)==cameFrom[newnodeid].end()){ 
      ...
      */
      ...
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
          ...
          Cost focalHeuristic = current.focalHeuristic +
...
void star_t(swarm::Timestamp tt, int si){
  ...
  swarm::enqueue(main_loop_task_para,0 , EnqFlags::NOHINT, si);
	...
	void search(void) {
		...
		printf("[DEBUG] sizeof(HighLevelNode) 21k = %lu\n", sizeof(start)); 
    ...
    printf("[DEBUG] sizeof(unord_map) = %lu\n", sizeof(std::unordered_map<State, std::tuple<State, Action, Cost, Cost>,StateHasher>));
    ...
    for (size_t i = 0; i < g_initialStates.size(); ++i) {
      ...
      if (i < ROBOT_NUMS && g_solution[i].states_size > 1) {
        ...
        LowLevelEnvironment llenv(g_env, i, start.constraints[i], start.solution);
